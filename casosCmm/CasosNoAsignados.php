...
<head>
   ...
   <script> 
        ...
        function newanalista(p_id){
            analista =document.getElementById("analistad").value;
            var id = p_id;
            $.ajax({
                    url: "funcion_analistadb.php",
                    method: "POST",
                    async: false,
                    data: {bdid : id, newanalista : analista  },
                    //dataType: "json", 
                        success: function(respuesta) {
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="table-responsive">  
                        <h1>Casos No Asignados</h1>  <br><br>    
                            ...
                            ?>
                                        <tr>
                                            ...
                                            <td>
                                                <div class="tab" >
                                                    <select name="valor9" id="analistad" onchange="newanalista(<?php echo $fila['id']; ?>)">
                                                        <?php 
                                                            $db =  new database();
                                                                $quer = $db->connect()->prepare("SELECT CONCAT(nombre, ' ', apellido) Analistas FROM usuarios WHERE tipo_usuario = 2 OR tipo_usuario = 3 ");
                                                                $quer -> execute();
                                                                $row = $quer->fetchAll(); 
                                                                echo "<option hidden selected> Seleccione </option>";
                                                                foreach ($row as $valores):
                                                                    echo '<option value="'.$valores["Analistas"].'" ';
                                                                        if ($valores["Analistas"] ==  $fila["analista"] ) 
                                                                            echo 'selected';//indicamos que quede seleccionada
                                                                            echo '>'.$valores["Analistas"].'</option>';                                            
                                                                endforeach;
                                                        ?>
                                                    </select>
                                                </div>
