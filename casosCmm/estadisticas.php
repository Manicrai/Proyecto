...
<?php
        ...
        require_once 'bdconnect.php';
        $db =  new database();
                $query = $db->connect()->prepare("SELECT cc.categoria, c.analista, e.estado, d.departamento, c.fecha_ingreso, c.fecha_actualizacion
                    FROM casos AS c
                ...
                if ($casosa && $query->rowCount()> 0)
                {
                    foreach ($casosa as $fila)
                    {
                        ...
                        <tr>
                            ...
                            <td style="display:none"><?php echo($fila["fecha_ingreso"]);?></td>              
                        </tr>
                <?php
                    }
                }
...
$.fn.dataTable.ext.search.push(
  ...
  series:[
    ...
    table.on('draw', function () {
        ...
        chart.series[1].setData(chartDat(table));
        chart.series[2].setData(chartDa(table));
    ...
    function chartData(table) {
        ...
        table.column(0, { search: 'applied' }).data().each(function (val) {            
