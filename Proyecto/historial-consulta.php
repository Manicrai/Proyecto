...
<body>
  ...
  <main id="main" class="main">
    <div class="pagetitle">
      <div class="col-md-4">
        <h1>Historial Consultas</h1>
        <label class="card-title">Paciente <?php echo $nombre ?></label> 
      </div>
      <form action="pacientes.php" method="POST">
        <div class="col-sm-5 offset-sm-2 col-md-7 offset-md-10">
          <button type="submit"  value="">Regresar</button>
        </div>  
      </form>
    </div>
    <!-- End Page Title -->
                  ...
                  <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
                    <thead>
                      ...
                      <?php
                        require_once 'bdconnect.php';
                          $db =  new database();
                          $query = $db->connect()->prepare("SELECT * FROM consulta_medica WHERE id_paciente = '$idpaciente'");
                          $query -> execute();
                      ...
                      ?>
                        <tr>
                          <td><span id="fecha<?php echo ($fila['id']);?>"><?php echo ($fila["fecha"]);?></span></td>
                          <td><span id="motivo_consulta<?php echo ($fila['id']);?>"><?php echo ($fila["motivo_consulta"]);?></span></td>
                          <td><span id="valor_consulta<?php echo ($fila['id']);?>"><?php echo ($fila["valor_consulta"]);?></span></td>
                          <td><span id="estado_pago<?php echo ($fila['id']);?>"><?php echo ($fila["estado_pago"]);?></span></td>
                          <td><button type="button" title="Ver Consulta" class="bi bi-archive cont"  value="<?php echo ($fila['id']);?>"></button>
                              <button type="button" title="Reportar Pago" class="bi bi-paperclip pago" value="<?php echo ($fila['id']);?>"></button>
                          </td>
                        </tr>
                        <?php
                                            }
